import json
with open(r'outputs/bisphenol A/step04_results.json', 'r', encoding='utf-8') as f: data = json.load(f)
...
for r in data:
    if r.get('dosage_status') in ['extracted', 'partial_data'] and r.get('doi', '').lower() not in skipped_dois:
        print('DOI:', r.get('doi'))
        print('Substitution Logic:', r.get('material_substitution_logic'))
        print('Metrics:', json.dumps(r.get('extracted_dosage_metrics', [])))
        print('---')
        count += 1
        if count == 5: break
